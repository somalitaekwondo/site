...
$query_email = mysql_query("SELECT * FROM DONATOR WHERE EMAIL LIKE '$email'");
	...
	/**THE EMAIL IS VALID**/
	$group="1";
	$namenewsletter=$name." ".$surname;
	
	$query_register=mysql_query("INSERT INTO DONATOR (NAME, SURNAME, BIRTHDAY, ADDRESS, NUMBER, ZIPCODE, CITY, STATE, COUNTRY, EMAIL, PASSWORD) VALUES ('$name', '$surname', '$birthday', '$address', '$number', '$zipcode', '$city', '$state', '$country', '$email', '$password')") or die (mysql_error());
	$query_newsletter=mysql_query("INSERT INTO MAIL_NEWSLETTER (MAIL_NEWSLETTER, ID_GROUP_NEWSLETTER, NAME_NEWSLETTER, DELETED) VALUES ('$email', '$group', '$namenewsletter', 'NO') ") or die (mysql_error());

		if(($query_register) and ($query_newsletter)){
...
exit;
		...
		header('Location: events?logregister=logregister');
...
$message="This message came from: ".$email." <br /><br />".$_SESSION['MESSAGE_CONTACT'];
...
if($idprocessing=="NEWSLETTER"){ /*******  NEWSLETTER ADD MAIL PAGE ************/
$newslettermail= $_SESSION['newslettermail'];
$newslettername = $_SESSION['newslettername'];
$group = "1";

	$query=mysql_query("INSERT INTO MAIL_NEWSLETTER (MAIL_NEWSLETTER, ID_GROUP_NEWSLETTER, NAME_NEWSLETTER, DELETED) VALUES ('$newslettermail', '$group', '$newslettername', 'NO') ") or die (mysql_error())	;
	if($query) header('Location: home?newsletter=add');
};
